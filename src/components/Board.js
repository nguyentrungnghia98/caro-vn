...
class Board extends React.Component {
  ...
  scrollToBottom(){
    this.stepMoveEl.scrollTop = this.stepMoveEl.scrollHeight - this.stepMoveEl.clientHeight;
  }
  ...
  renderRow(squares,i) {
    ...
    return this.size.map((el, j) => {
      ...
      return (
        <Square isHighLight={isHighLight} value={value} onClick={() => this.handleClick(i, j)} key={`square-${shortid.generate()}`} disabled={isWinner} />
      );
    });
  }
  ...
  renderMoveStep(history){
    ...
    return history.map((val, move) => {
      ...
      const desc = step? `Go to move #${  step  } (${location})`: 'Go to game start';
      return (
        <li key={`step${  step}`}>
          <button type="button" className={`btn btn-step ${  step === moveStep? 'current':''}`} onClick={() => this.moveToStep(step)}>{desc}</button>
        </li>
      );
    });
  }
  ...
  render() {
    ...
    return (
      <>
        <button type="button" className="btn btn-restart custom-location" onClick={this.onRestart}>
          Restart
        </button>

        {this.renderLeftSide(winner)}
        <div className="board">
          <PopUp toggle={toggle} onRestart={this.onRestart} onToggle={() => this.toggle('toggle', false)} winner={winner} />
          {this.renderBoard(squares)}
        </div>
        {this.renderRightSide(winner)}

        <div className="step-move-container">
          <button type="button"  className="btn btn-change" onClick={this.onChangeOrder}>Change order</button>
